...
from transformers import TrainingArguments, Trainer, AutoConfig, SamModel, SamProcessor
import numpy as np
import cv2
import argparse
...
from peft import load_peft_weights, set_peft_model_state_dict, PeftModel, PeftConfig
import json
import subprocess
        ...
        with torch.no_grad():
            for i, batch in enumerate(tqdm(eval_dataloader)):
                ...
                for ind in range(4):
                    ...
                    ax[2].set_title(f"LiteMedSAM\nDice: {dice_score.cpu():.2f} Jaccard: {jaccard_score.cpu():.2f}")
                    plt.show()
                    plt.savefig(f"output/LiteMedSAM/visualization/lora_{i}_{ind}.png")
                break
...
config = PeftConfig.from_pretrained("output/LiteMedSAM/checkpoint-600")
...
val_dataset = npyDataset(folder='val') # change to test for test scores
